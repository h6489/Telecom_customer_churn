...
model = pickle.load(open('etc_model.pkl', 'rb'))
...
def prediction():
    if request.method == 'POST':
        ...
        total_og_weightage = float(request.form.get('total_og_weightage'))
        ...
        aon = float(request.form.get('aon'))
        total_rech_num_8 = float(request.form.get('total_rech_num_8'))
        arpu_8 = float(request.form.get('arpu_8'))
        max_rech_amt_8 = float(request.form.get('max_rech_amt_8'))
